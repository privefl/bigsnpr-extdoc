...
PLINK is very efficient for conversion and quality control of multiple formats, so I provide some wrappers to PLINK in {bigsnpr}, for ease of use directly from `r icon::fa_r_project()`:
...
zip <- runonce::download_file(
  "https://figshare.com/ndownloader/files/36597108",
...
```
We can store some extra information on the individuals (e.g. some phenotypes):
```{r}
