...
# would better to have a per-variant sample size in the sumstats
...
for (chr in 1:4) {
  ...
  if (chr == 1) {
    ...
    corr <- as_SFBM(corr0, "tmp-data/corr")
  } else {
...
```{r}
(params <- expand.grid(p = signif(seq_log(0.001, 0.2, length.out = 8), 2),
...
pred_grid <- big_prodMat(G, beta_grid, ind.col = df_beta[["_NUM_ID_"]],
                         ncores = NCORES)
params$score <- big_univLogReg(as_FBM(pred_grid), info$CAD)$score
```
...
multi_auto <- snp_ldpred2_auto(
  ...
  vec_p_init = seq_log(1e-3, 0.1, 4),  # TO CHANGE
...
lassosum2 is a re-implementation of the lassosum model that now uses the exact same input parameters as LDpred2 (`corr` and `df_beta`). It should be much faster than running LDpred2. It can be run next to LDpred2 and the best model can be chosen using the validation set.

```{r}
beta_lassosum2 <- snp_lassosum2(
  corr, df_beta, ncores = NCORES,
  s = c(0.2, 0.5, 0.8, 1), nlambda = 10, maxiter = 50)  # TO REMOVE
```
...
pred_grid2 <- big_prodMat(G, beta_lassosum2, ind.col = df_beta[["_NUM_ID_"]],
                          ncores = NCORES)
params2$score <- big_univLogReg(as_FBM(pred_grid2), info$CAD)$score
```

```{r}
library(ggplot2)
ggplot(params2, aes(x = lambda, y = score, color = as.factor(s))) +
  theme_bigstatsr() +
  geom_point() +
  geom_line() +
