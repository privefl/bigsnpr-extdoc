...
All the elements of an FBM have the same type; supported types are:
 ...
 - `"integer"` (signed, so between $\text{-}2^{31}$ and $2^{31} - 1$)
...
Note that the elements of the FBM are stored column-wise (as for a standard R matrix). Therefore, be careful not to access a subset of rows, since it would read non-contiguous elements from the whole matrix from disk. [TODO: MAKE NEW CHAPTER ON THIS]
::::
...
- To read any format used in genetics, you can always convert blocks of the data to text files using PLINK, read these using `bigreadr::fread2()`, and fill part of the resulting FBM. [TODO: export the code to convert from RICOPILI imputed data]
